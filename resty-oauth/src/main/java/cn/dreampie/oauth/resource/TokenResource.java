...
public class TokenResource extends Resource {
   ...
   */
  @POST
  public ApiResponse getToken(Client client, String code, String grant_type) {
    String redirectParam = "?key=" + client.get("key") + "&code=" + code + "&grant_type=" + grant_type;
    if (!grant_type.equals("authorization_code")) {
      return new ApiResponse(HttpStatus.FOUND, Maper.<String, String>of("location", Constant.oauthErrorUrl + redirectParam + "&error=grant_type_not_match"));
    }
    ...
    if (oldClient == null || c == null) {
      return new ApiResponse(HttpStatus.FOUND, Maper.<String, String>of("location", Constant.oauthErrorUrl + redirectParam + "&error=client_or_code_not_found"));
    } else {
      //判断账号是否支持该授权方式
      if (!Arrays.asList(oldClient.getGrant().split(",")).contains(grant_type)) {
        return new ApiResponse(HttpStatus.FOUND, Maper.<String, String>of("location", Constant.oauthErrorUrl + redirectParam + "&error=grant_type_not_support"));
      }
      ...
      return new ApiResponse(HttpStatus.OK, accessToken);
    }
  }

  @PUT
  public ApiResponse refreshToken(String refresh_token, String grant_type) {
    ...
    if (token == null) {
      return new ApiResponse(HttpStatus.FOUND, Maper.<String, String>of("location", Constant.oauthErrorUrl + redirectParam + "&error=token_not_found"));
    } else {
