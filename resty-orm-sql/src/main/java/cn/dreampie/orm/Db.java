...
public class Db {
  ...
  public static Db use(String dbName) {
    Db db = new Db();
    db.connectionMetadata = Connections.getConn(dbName);
    checkNotNull(db.connectionMetadata);
    return db;
  }
  ...
  private PreparedStatement getPreparedStatement(String sql, Object[] paras) throws SQLException {
    PreparedStatement pst = connectionMetadata.getConnection().prepareStatement(sql);
  ...
  List<Record> find(String sql, Object... paras) throws SQLException {
    PreparedStatement pst = getPreparedStatement(sql, paras);
    ResultSet rs = pst.executeQuery();
